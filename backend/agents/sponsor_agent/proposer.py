"""
Sponsor Proposal Generator -- uses Groq to create personalised sponsorship proposals.
...
def _call_groq_with_retry(client, model: str, prompt: str) -> str:
        ...
        try:
            ...
            if "limit: 0" in error_str:
                ...
                if "JSON array" in prompt:
                    ...
                    return "This is a mock proposal. The Groq API key provided has exceeded its quota or is disabled in your region."
            elif "429" in error_str or "rate limit" in error_str.lower() or "RESOURCE_EXHAUSTED" in error_str:
    ...
    print("   [ERROR] Max retries exceeded for API rate limit. Falling back.")
    if "JSON array" in prompt:
        return '[]'
...
class ProposalGenerator:
    ...
    def enrich_sponsors(self, sponsors: List[Sponsor]) -> List[Sponsor]:
        ...
        by sending company names to Groq in batches.
        """
...
Do NOT include any text before or after the JSON array."""
            ...
            try:
                text = _call_groq_with_retry(self.client, self.model, prompt)
                ...
                enriched_map = {
                    e.get("company_name", "").lower(): e for e in enriched
                }
...
Write the proposal as a formal letter/email body. Do NOT include subject line or headers."""
        ...
        try:
            return _call_groq_with_retry(self.client, self.model, prompt)
        except Exception as e:
